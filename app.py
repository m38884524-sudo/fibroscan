...
# --- Spleen ---
st.subheader("Spleen")
sp_length = st.number_input("Spleen length (mm)", min_value=0.0, format="%.1f", value=None)
sp_width  = st.number_input("Spleen width (mm)", min_value=0.0, format="%.1f", value=None)


# --- Liver stiffness ---
st.subheader("Liver Stiffness")
ls2 = st.number_input("LS2", min_value=0.0, format="%.1f", value=None)


# --- Portal vein ---
st.subheader("Portal Vein")
pv_caliber = st.number_input("Portal vein caliber (mm)", min_value=0.0, format="%.1f", value=None)
pv_velocity = st.number_input("Portal vein velocity (cm/s)", min_value=0.0, format="%.1f", value=None)


# --- Liver segments (text input with 'x' separator) ---
...
if st.button("Evaluate"):
    ...
    if sp_length is None or sp_width is None or ls2 is None or \
       pv_caliber is None or pv_velocity is None:
        st.error("Please fill in all number fields before evaluating.")
        st.stop()
    ...
    if not (seg5_values and seg8_values and seg7_values):
        ...
        st.markdown(f"**Liver stiffness (LS2):** {patient['liver']['ls2']:.1f}")
